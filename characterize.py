...
class Characterize(ServiceBase):
    ...
    def execute(self, request: ServiceRequest) -> None:
        ...
        if request.file_type.startswith("uri/"):
            self.handle_uri(request)
            return

        # 1. Calculate entropy map
        self.calculate_entropy(request)

        if request.file_type != "shortcut/windows":
            # 2. Get hachoir metadata
            self.execute_hachoir(request)

        # 3. Get Exiftool Metadata
        self.execute_exiftool(request)

        # 4. Lnk management.
        if request.file_type == "shortcut/windows":
            self.handle_windows_shortcut(request)

        # 5. URL file management
        if request.file_type == "shortcut/web":
            self.handle_web_shortcut(request)

    def handle_uri(self, request: ServiceRequest):
    ...
    def calculate_entropy(self, request: ServiceRequest):
    ...
    def execute_hachoir(self, request: ServiceRequest):
    ...
    def execute_exiftool(self, request: ServiceRequest):
        ...
        if exif.stdout:
            ...
            if "Error" not in res_data:
                ...
                if exif_body:
                    ...
                    current_tag_map = copy.deepcopy(TAG_MAP.get(None, {}))
    ...
    def handle_windows_shortcut(self, request: ServiceRequest):
        try:
    ...
    def handle_web_shortcut(self, request: ServiceRequest):
        ...
        if b"\x00" in file_contents:
            with tempfile.NamedTemporaryFile(dir=self.working_directory, delete=False) as fh:
                fh.write(file_contents.replace(b"\x00", b""))
            file_path = fh.name
            heur = Heuristic(6)
            heur_section = ResultSection(heur.name, heuristic=heur, parent=res, auto_collapse=True)
            null_count = file_contents.count(b"\x00")
            heur_section.add_line(f"Removed {null_count} null bytes from shortcut.")
